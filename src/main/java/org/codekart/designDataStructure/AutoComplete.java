...
public class AutoComplete {
     ...
     * the hottest one). If several sentences have the same hot degree, use
     ...
     * List<String> input(char c) This indicates that the user typed the character
     * c.
     ...
     * Each tested sentence will be a sequence o f characters c that end with the
     * character '#'.
     ...
     * code 114, "i love leetcode" should be in front of "ironman". Also we only
    ...
    public List<String> input(char c) {
        ...
        Collections.sort(result, (a, b) -> {
            ...
            return a.compareTo(b); // Ascending ASCII order
        });
    ...
    private void dfs(TrieNode node, String currentPrefix, List<String> result) {
        if (countMap.containsKey(currentPrefix)) {
            result.add(currentPrefix);
        }

        for (char c : node.children.keySet()) {
            dfs(node.children.get(c), currentPrefix + c, result);
        }
    }
